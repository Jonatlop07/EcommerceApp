...
import { ValidationPipe } from './/api/http-rest/common/pipes/validation.pipe';

function buildAPIDocumentation(app: INestApplication): void {
  const title = 'Mini Ecommerce Cart Microservice';
  const description = 'Mini Ecommerce Cart Microservice API Documentation';
  const version = '1.0.0';
  const tag = 'cart';
